...
PlaygroundBuilder.prototype._createPlaygroundFile = function(outputFile, callback) {
  ...
  // Write section files (documentation and code)
  this.sections.forEach(function(section) {
    // Write documentation
    if (section.type === 'documentation') {
      var html = this._renderHTMLForSection(section);
      writeFile(path.join(outputFile, 'Documentation', section.filename), html);
    }
    // Write Swift code
    else if (section.type === 'code') {
      writeFile(path.join(outputFile, section.filename), section.content);
    }
  }.bind(this));

  // Write XML manifest file containing references to the docs and code
  writeFile(path.join(outputFile, 'contents.xcplayground'), this._getPlaygroundXML());
  ...
  function writeFile(filename, content) {
    fs.outputFile(filename, content, function(err) {
      ...
      writesComplete++;
      if (writesComplete === numSections + 1 /* extra 1 is for .xcplayground file */) {
        callback();
      }
    });
  }
...
PlaygroundBuilder.prototype.createPlayground = function(inputFile, outputDirectory, callback) {
  ...
  var outputFile = path.join(outputDirectory, inputFile.substr(0, inputFile.lastIndexOf('.')));
